...
class TestHighScore(unittest.TestCase):
    ...
    # ---------- 1. load_scores ----------
    @patch("builtins.open", new_callable=mock_open, read_data=json.dumps({"x": {"name": "Test"}}))
    ...
    def test_get_or_create_player_existing_case_insensitive(self, mock_save):
        hs = HighScore()
        hs.scores = {"123": {"name": "Alice", "total_games": 1, "wins": 0, "highest_score": 20}}
    ...
    def test_rename_player_no_conflict(self, mock_save):
        hs = HighScore()
        hs.scores = {"111": {"name": "OldName", "total_games": 1, "wins": 0, "highest_score": 10}}
    ...
    def test_record_game_win(self, mock_save):
        hs = HighScore()
        hs.scores = {"id1": {"name": "Alice", "total_games": 1, "wins": 0, "highest_score": 40}}
    ...
    def test_record_game_highest_score_update(self, mock_save):
        hs = HighScore()
        hs.scores = {"id1": {"name": "Bob", "total_games": 0, "wins": 0, "highest_score": 90}}
        hs.record_game("id1", 50, won=False)
        self.assertEqual(hs.scores["id1"]["highest_score"], 90)  # no update since 50 < 90
    ...
    def test_record_game_nonexistent(self, mock_save):
        hs = HighScore()
        hs.scores = {}
        hs.record_game("ghost", 50, won=True)  # should not crash
        self.assertEqual(hs.scores, {})
